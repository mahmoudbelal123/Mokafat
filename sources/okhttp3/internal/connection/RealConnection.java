...
public final class RealConnection extends Http2Connection.Listener implements Connection {
    ...
    private void connectTls(ConnectionSpecSelector connectionSpecSelector) throws IOException {
        ...
        try {
            try {
                maybeProtocol = (SSLSocket) sslSocketFactory.createSocket(this.rawSocket, address.url().host(), address.url().port(), true);
                ConnectionSpec connectionSpec = connectionSpecSelector.configureSecureSocket(maybeProtocol);
                if (connectionSpec.supportsTlsExtensions()) {
                    Platform.get().configureTlsExtensions(maybeProtocol, address.url().host(), address.protocols());
                }
                maybeProtocol.startHandshake();
                Handshake unverifiedHandshake = Handshake.get(maybeProtocol.getSession());
                if (!address.hostnameVerifier().verify(address.url().host(), maybeProtocol.getSession())) {
