...
final class RealBufferedSource implements BufferedSource {
    ...
    public String readUtf8LineStrict(long limit) throws IOException {
        ...
        if (scanLength < LongCompanionObject.MAX_VALUE && request(scanLength) && this.buffer.getByte(scanLength - 1) == 13 && request(scanLength + 1) && this.buffer.getByte(scanLength) == 10) {
    ...
    public boolean rangeEquals(long offset, ByteString bytes, int bytesOffset, int byteCount) throws IOException {
        ...
        for (int i = 0; i < byteCount; i++) {
            ...
            if (!request(bufferOffset + 1) || this.buffer.getByte(bufferOffset) != bytes.getByte(bytesOffset + i)) {
                return false;
            }
        }
        return true;
