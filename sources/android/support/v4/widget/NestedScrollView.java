...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent, NestedScrollingChild2, ScrollingView {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode != 0 && getChildCount() > 0) {
            ...
            if (child.getMeasuredHeight() < height) {
                ...
                int childHeightMeasureSpec = View.MeasureSpec.makeMeasureSpec((height - getPaddingTop()) - getPaddingBottom(), CrashUtils.ErrorDialogData.SUPPRESSED);
    ...
    public boolean executeKeyEvent(@NonNull KeyEvent event) {
        ...
        if (!zCanScroll) {
            ...
            return (nextFocused == null || nextFocused == this || !nextFocused.requestFocus(BuildConfig.VERSION_CODE)) ? false : true;
        }
        ...
        if (keyCode != 62) {
            switch (keyCode) {
                ...
                case 20:
                    if (!event.isAltPressed()) {
                        boolean handled3 = arrowScroll(BuildConfig.VERSION_CODE);
                    } else {
                        boolean handled4 = fullScroll(BuildConfig.VERSION_CODE);
                    }
                    break;
            }
            return false;
