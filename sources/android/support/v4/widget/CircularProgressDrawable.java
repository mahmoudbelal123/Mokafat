    ...
    public void applyTransformation(float interpolatedTime, Ring ring, boolean lastFrame) {
        ...
        if (interpolatedTime != 1.0f || lastFrame) {
            ...
            if (interpolatedTime >= SHRINK_OFFSET) {
                float scaledTime = (interpolatedTime - SHRINK_OFFSET) / SHRINK_OFFSET;
                startTrim = ring.getStartingStartTrim() + 0.79f;
                startTrim2 = startTrim - ((0.79f * (1.0f - MATERIAL_INTERPOLATOR.getInterpolation(scaledTime))) + MIN_PROGRESS_ARC);
            } else {
                float scaledTime2 = interpolatedTime / SHRINK_OFFSET;
                float startTrim3 = ring.getStartingStartTrim();
                float endTrim = (0.79f * MATERIAL_INTERPOLATOR.getInterpolation(scaledTime2)) + MIN_PROGRESS_ARC + startTrim3;
