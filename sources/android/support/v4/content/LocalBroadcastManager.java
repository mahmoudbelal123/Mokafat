...
public final class LocalBroadcastManager {
    ...
    public boolean sendBroadcast(@NonNull Intent intent) {
        ...
        synchronized (this.mReceivers) {
            ...
            ArrayList<ReceiverRecord> entries2 = this.mActions.get(intent.getAction());
            if (entries2 != null) {
                if (debug) {
                    Log.v(TAG, "Action list: " + entries2);
                }
                ArrayList<ReceiverRecord> receivers2 = null;
                int i2 = 0;
                while (true) {
                    int i3 = i2;
                    int i4 = entries2.size();
                    if (i3 < i4) {
                        ReceiverRecord receiver = entries2.get(i3);
                        if (debug) {
                            StringBuilder sb = new StringBuilder();
                            i = i3;
                            sb.append("Matching against filter ");
                            sb.append(receiver.filter);
                        ...
                        if (receiver.broadcasting) {
                            ...
                            if (match >= 0) {
                                ...
                                receivers2 = receivers == null ? new ArrayList<>() : receivers;
                                ...
                                switch (match) {
                                    ...
                                    case FontsContractCompat.FontRequestCallback.FAIL_REASON_FONT_LOAD_ERROR /* -3 */:
                                        ...
                                        break;
                                    case -2:
                                        reason = DataBufferSafeParcelable.DATA_FIELD;
                                        break;
                                    case -1:
                                        reason = AppMeasurement.Param.TYPE;
                                        break;
                                    default:
                        ...
                        if (receivers3 != null) {
                            for (int i5 = 0; i5 < receivers3.size(); i5++) {
                                receivers3.get(i5).broadcasting = false;
                            }
