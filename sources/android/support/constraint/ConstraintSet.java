...
public class ConstraintSet {
    ...
    public void clone(ConstraintSet set) {
        ...
        for (Integer key : set.mConstraints.keySet()) {
            this.mConstraints.put(key, set.mConstraints.get(key).m0clone());
        }
    }
    ...
    public void createVerticalChain(int topId, int topSide, int bottomId, int bottomSide, int[] chainIds, float[] weights, int style) {
        ...
        while (true) {
            int i2 = i;
            int i3 = chainIds.length;
            if (i2 >= i3) {
                int i4 = chainIds.length;
                connect(chainIds[i4 - 1], 4, bottomId, bottomSide, 0);
                return;
            }
            int i5 = chainIds[i2];
            ...
            if (weights != null) {
                get(chainIds[i2]).verticalWeight = weights[i2];
            }
            i = i2 + 1;
        }
    }
    ...
    private void createHorizontalChain(int leftId, int leftSide, int rightId, int rightSide, int[] chainIds, float[] weights, int style, int left, int right) {
        ...
        while (true) {
            ...
            if (i2 >= i3) {
                int i4 = chainIds.length;
                connect(chainIds[i4 - 1], right, rightId, rightSide, -1);
                return;
            }
            int i5 = chainIds[i2];
            ...
            if (weights != null) {
                get(chainIds[i2]).horizontalWeight = weights[i2];
            }
            i = i2 + 1;
        }
    }
    ...
    public void load(Context context, int resourceId) throws XmlPullParserException, IOException {
