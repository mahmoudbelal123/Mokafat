...
final class Base64 {
    ...
    public static byte[] decode(String in) {
        ...
        int word = 0;
        int outCount = 0;
        for (int outCount2 = 0; outCount2 < limit; outCount2++) {
            char c2 = in.charAt(outCount2);
            if (c2 >= 'A' && c2 <= 'Z') {
                bits = c2 - 'A';
            } else if (c2 >= 'a' && c2 <= 'z') {
                bits = c2 - 'G';
        ...
        if (lastWordChars == 2) {
            ...
            outCount = outCount5 + 1;
            out[outCount5] = (byte) (word2 >> 8);
        }
        int outCount6 = out.length;
        if (outCount == outCount6) {
            return out;
        }
