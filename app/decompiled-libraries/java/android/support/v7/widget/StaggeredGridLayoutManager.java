...
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    /* JADX WARN: Type inference failed for: r22v0, types: [android.support.v7.widget.StaggeredGridLayoutManager] */
    ...
    private int fill(RecyclerView.Recycler recycler, LayoutState layoutState, RecyclerView.State state) {
        ...
        while (true) {
            ...
            ?? r17 = span2 == -1 ? r15 : r14;
            if (r17 != 0) {
                nextSpan = layoutParams2.mFullSpan ? this.mSpans[r14] : getNextSpan(layoutState);
                this.mLazySpanLookup.setSpan(viewLayoutPosition, nextSpan);
            } else {
                nextSpan = this.mSpans[span2];
            }
            Span span3 = nextSpan;
            layoutParams2.mSpan = span3;
            if (layoutState.mLayoutDirection == r15) {
                addView(next);
            } else {
                addView(next, r14);
            }
            measureChildWithDecorationsAndMargin(next, layoutParams2, r14);
            if (layoutState.mLayoutDirection == r15) {
                int maxEnd = layoutParams2.mFullSpan ? getMaxEnd(endAfterPadding) : span3.getEndLine(endAfterPadding);
                int decoratedMeasurement3 = this.mPrimaryOrientation.getDecoratedMeasurement(next) + maxEnd;
                if (r17 != 0 && layoutParams2.mFullSpan) {
                ...
                int minStart = layoutParams2.mFullSpan ? getMinStart(endAfterPadding) : span3.getStartLine(endAfterPadding);
                int decoratedMeasurement4 = minStart - this.mPrimaryOrientation.getDecoratedMeasurement(next);
                if (r17 != 0 && layoutParams2.mFullSpan) {
                    LazySpanLookup.FullSpanItem fullSpanItemCreateFullSpanItemFromStart = createFullSpanItemFromStart(minStart);
                    fullSpanItemCreateFullSpanItemFromStart.mGapDir = r15;
            ...
            if (layoutParams2.mFullSpan && layoutState.mItemDirection == -1) {
                if (r17 != 0) {
                    ...
                    if ((layoutState.mLayoutDirection == r15 ? (areAllEndsEqual() ? 1 : 0) ^ r15 : (areAllStartsEqual() ? 1 : 0) ^ r15) != 0) {
                        LazySpanLookup.FullSpanItem fullSpanItem = this.mLazySpanLookup.getFullSpanItem(viewLayoutPosition);
                        if (fullSpanItem != null) {
                            fullSpanItem.mHasUnwantedGapAfter = r15;
                        }
                        this.mLaidOutInvalidFullSpan = r15;
                    }
                }
            }
            attachViewToSpans(next, layoutParams2, layoutState);
            if (isLayoutRTL() && this.mOrientation == r15) {
                int endAfterPadding2 = layoutParams2.mFullSpan ? this.mSecondaryOrientation.getEndAfterPadding() : this.mSecondaryOrientation.getEndAfterPadding() - (((this.mSpanCount - r15) - span3.mIndex) * this.mSizePerSpan);
                decoratedMeasurement2 = endAfterPadding2;
                decoratedMeasurement = endAfterPadding2 - this.mSecondaryOrientation.getDecoratedMeasurement(next);
            } else {
                int startAfterPadding = layoutParams2.mFullSpan ? this.mSecondaryOrientation.getStartAfterPadding() : (span3.mIndex * this.mSizePerSpan) + this.mSecondaryOrientation.getStartAfterPadding();
                decoratedMeasurement = startAfterPadding;
                decoratedMeasurement2 = this.mSecondaryOrientation.getDecoratedMeasurement(next) + startAfterPadding;
            }
            if (this.mOrientation == r15) {
                span = span3;
                layoutDecoratedWithMargins(next, decoratedMeasurement, i2, decoratedMeasurement2, i);
                ...
                i3 = endAfterPadding;
                i4 = i5;
                layoutDecoratedWithMargins(next, i2, decoratedMeasurement, i, decoratedMeasurement2);
            }
        ...
        return startAfterPadding2 > 0 ? Math.min(layoutState.mAvailable, startAfterPadding2) : r7 == true ? 1 : 0;
    }
    ...
    public void collectAdjacentPrefetchPositions(int dx, int dy, RecyclerView.State state, RecyclerView.LayoutManager.LayoutPrefetchRegistry layoutPrefetchRegistry) {
        ...
        for (int itemPrefetchCount2 = 0; itemPrefetchCount2 < this.mSpanCount; itemPrefetchCount2++) {
            int distance = this.mLayoutState.mItemDirection == -1 ? this.mLayoutState.mStartLine - this.mSpans[itemPrefetchCount2].getStartLine(this.mLayoutState.mStartLine) : this.mSpans[itemPrefetchCount2].getEndLine(this.mLayoutState.mEndLine) - this.mLayoutState.mEndLine;
            if (distance >= 0) {
    ...
    public View onFocusSearchFailed(View focused, int direction, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        int referenceChildPosition = layoutDir == 1 ? getLastChildPosition() : getFirstChildPosition();
        updateLayoutState(referenceChildPosition, state);
        setLayoutStateDirection(layoutDir);
        this.mLayoutState.mCurrentPosition = this.mLayoutState.mItemDirection + referenceChildPosition;
        ...
        if (!prevFocusFullSpan) {
            View unfocusableCandidate = findViewByPosition(shouldSearchFromStart ? prevFocusSpan.findFirstPartiallyVisibleItemPosition() : prevFocusSpan.findLastPartiallyVisibleItemPosition());
        ...
        if (!preferLastSpan(layoutDir)) {
            while (true) {
                ...
                View unfocusableCandidate2 = findViewByPosition(shouldSearchFromStart ? this.mSpans[i4].findFirstPartiallyVisibleItemPosition() : this.mSpans[i4].findLastPartiallyVisibleItemPosition());
            ...
            while (true) {
                ...
                if (i7 != prevFocusSpan.mIndex) {
                    View unfocusableCandidate3 = findViewByPosition(shouldSearchFromStart ? this.mSpans[i7].findFirstPartiallyVisibleItemPosition() : this.mSpans[i7].findLastPartiallyVisibleItemPosition());
